...
// --- ENHANCEMENT: Function to invalidate a specific user's cache ---
// This will be called from other controllers (like FastingController) when data changes.
...
// --- ENHANCEMENT: Helper function to format duration ---
...
exports.getStats = async (req, res) => {
  ...
  // --- ENHANCEMENT: Check for force refresh header from api.js ---
  ...
  try {
    ...
    logger.info(`DASHBOARD_CTRL: Cache MISS${forceRefresh ? ' or FORCE REFRESH' : ''} for ${cacheKey}. Fetching from DB.`);
    ...
    const dataPromise = (async () => {
      ...
      // --- ENHANCEMENT: Use the formatting function ---
      const longestFastFormatted = formatMilliseconds(longestFastDoc?.elapsedTime);
      logger.debug(`DASHBOARD_CTRL: Calculated longest fast: ${longestFastFormatted}`);
      ...
      await Dashboard.findOneAndUpdate(
        { user: userObjectId },
        {
          $set: {
            totalFasts,
            longestFast: Math.round((longestFastDoc?.elapsedTime || 0) / 3600000), // Keep hours for dashboard document
      ...
      const responseData = {
        // The frontend uses `user.streak` and `user.badges` from the AuthContext, which gets refreshed.
        // The main purpose of this endpoint is to provide the aggregate stats.
        stats: {
          totalFasts: totalFasts || 0,
          longestFast: longestFastFormatted || 'N/A',
        },
        // We still send user data in case it's needed, but frontend relies on its own user state
        user: {
