...
const MAX_FASTING_HOURS = 6720; // 7 days
...
const updateUserAchievements = async (userId, completedFast) => {
    const now = new Date(completedFast.endTime);
    const userIdObj = new mongoose.Types.ObjectId(userId);
    try {
        const user = await User.findById(userIdObj).select('streak lastFastEndTime badges').lean();
        if (!user) {
            logger.warn(`[updateUserAchievements] User not found: ${userId}`);
            return null;
        }
        let updateOps = {};
        if (!user.lastFastEndTime || (now.getTime() - new Date(user.lastFastEndTime).getTime()) > STREAK_RESET_HOURS * 60 * 60 * 1000) {
            updateOps.streak = 1;
        } else {
            updateOps.$inc = { streak: 1 };
        }
        updateOps.lastFastEndTime = now;
        const newStreak = updateOps.streak || (user.streak + 1);
        const currentBadges = user.badges || [];
        const badgesToAdd = [];
        if (!currentBadges.includes('First Fast')) {
            const totalCompleted = await Fast.countDocuments({ user: userIdObj, completed: true });
            if (totalCompleted === 1) badgesToAdd.push('First Fast');
        }
        if (newStreak >= 7 && !currentBadges.includes('Weekly Warrior')) badgesToAdd.push('Weekly Warrior');
        if (newStreak >= 30 && !currentBadges.includes('Monthly Master')) badgesToAdd.push('Monthly Master');
        const completedHours = completedFast.elapsedTime / 3600000;
        if (completedHours >= 24 && !currentBadges.includes('24-Hour Club')) badgesToAdd.push('24-Hour Club');
        if (completedHours >= 48 && !currentBadges.includes('48-Hour Hero')) badgesToAdd.push('48-Hour Hero');
        if (completedHours >= 72 && !currentBadges.includes('72-Hour Titan')) badgesToAdd.push('72-Hour Titan');
        if (badgesToAdd.length > 0) {
            updateOps.$addToSet = { badges: { $each: badgesToAdd } };
        }
        if (Object.keys(updateOps).length > 0) {
            logger.info(`[updateUserAchievements] Applying updates for user ${userId}:`, updateOps);
            await User.findByIdAndUpdate(userIdObj, updateOps, { new: true }).lean();
        }
    } catch (error) {
        logger.error(`[updateUserAchievements] Error for user ${userId}:`, error);
    }
};
...
const FastingController = {
    ...
    // In controllers/FastingController.js
      async endFast(req, res) {
        ...
        try {
            ...
            const userId = req.user.id || req.user._id;
            const userIdStr = userId.toString();
            const fast = await Fast.findOne({ _id: fastId, user: userIdStr });

            if (!fast || !fast.isRunning) {
                return res.status(404).json({ success: false, message: 'Active fast not found.' });
            }
            ...
            fast.elapsedTime = Math.max(0, now.getTime() - new Date(fast.startTime).getTime());
            ...
            logger.info(`${logPrefix} Saved updated fast doc ${savedFast._id}.`);
            
            await updateUserAchievements(userIdStr, savedFast);
            logger.info(`${logPrefix} Achievements updated.`);

            // This line will now work correctly
            await invalidateUserCache(userIdStr);
            logger.info(`${logPrefix} Dashboard cache invalidated.`);

            res.json({ success: true, message: 'Fast ended successfully.', fast: savedFast.toObject() });

        } catch (error) {
            logger.error(`${logPrefix} Error caught:`, error);
            res.status(500).json({ success: false, message: 'Server error while ending fast.' });
        }
    },
    // ... all other methods remain the same ...
     async getCurrentFast(req, res) {
        try {
            ...
            const targetHours = currentFast.targetHours || 16;
    ...
    async getDashboardStats(req, res) {
        try {
            ...
            // The helper function from dashboardController to format time
            ...
            const [statsAgg, user, recentFasts] = await Promise.all([
                Fast.aggregate([
                    { $match: { user: new mongoose.Types.ObjectId(userId), completed: true, elapsedTime: { $exists: true, $gt: 0 } } },
                    { $group: { _id: null, totalFasts: { $sum: 1 }, longestFastMs: { $max: '$elapsedTime' } } },
                    { $project: { _id: 0, totalFasts: 1, longestFast: '$longestFastMs' } }
                ]),
                User.findById(userId).select('streak badges').lean(),
                Fast.find({ user: userId, completed: true }, { endTime: 1, elapsedTime: 1 }, { sort: { endTime: -1 }, limit: 7, lean: true })
            ]);

            const basicStats = statsAgg[0] || { totalFasts: 0, longestFast: 0 };
            
            const dashboardData = {
                success: true,
                stats: {
                    totalFasts: basicStats.totalFasts,
                    longestFast: formatMilliseconds(basicStats.longestFast),
                },
                streak: user?.streak || 0,
                badges: user?.badges || [],
                recentFasts: recentFasts.map(fast => ({
                     date: fast.endTime.toISOString().split('T')[0],
                     duration: fast.elapsedTime ? parseFloat((fast.elapsedTime / 3600000).toFixed(1)) : 0
                 })).reverse()
            };
